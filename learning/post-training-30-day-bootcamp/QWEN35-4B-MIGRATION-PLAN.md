...
关闭处置：`not_executed_superseded`。Day 20/RSI 使用独立六源数据合同；原 7,860 条 manifest v2 和独立 selection/confirmation split 不存在。RSI 实际合同已声明同一 full112 comparison identity 与独立训练 seed；它不是新 held-out split。
...
- 只有 RSI selection decision、但尚无 recovered/export-parity-verified S1 manifest 时尝试 DPO/GRPO；
